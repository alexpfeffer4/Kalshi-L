...
function splitCaseTitle(value) {
  const match = (value || "").match(/^\s*(.+?)\s+v[.\s]\s+(.+?)\s*$/i);
...
function inferStructuredFields(input) {
  ...
  const sides = input.sourceType === "court" ? splitCaseTitle(sourceDetails.caseName || sourceDetails.caseNameShort || input.title) : null;
  return {
    plaintiff: normalizeStructuredText(input.plaintiff || sourceDetails.plaintiff || sides?.plaintiff),
    defendant: normalizeStructuredText(input.defendant || sourceDetails.defendant || sides?.defendant),
    ...
    sourcePublishedAt: normalizeStructuredDate(
      input.sourcePublishedAt || sourceDetails.sourcePublishedAt || sourceDetails.publishedAt || input.publishedAt || input.detectedAt
    ),
    legalPosture: normalizeStructuredText(input.legalPosture || sourceDetails.legalPosture),
  };
}

export function inferStructuredFieldsFromEvent(event) {
  return inferStructuredFields(event);
}
