...
function mapEventToRow(event) {
  return {
    ...
    source_details: event.sourceDetails || {},
    plaintiff: event.plaintiff || "",
    defendant: event.defendant || "",
    court: event.court || "",
    agency: event.agency || "",
    docket_number: event.docketNumber || "",
    source_published_at: event.sourcePublishedAt || null,
    legal_posture: event.legalPosture || "",
...
export async function recordEventAuditLogSupabase(entry) {
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.from("event_audit_logs").insert({
...
export async function listEventAuditLogsSupabase(eventId) {
  const supabase = createSupabaseServerClient();
  const { data, error } = await supabase
