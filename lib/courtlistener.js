...
function looksLikeOrganization(name) {
  return /\b(llc|inc|corp|corporation|company|ltd|lp|llp|pllc|association|bank|holdings)\b/i.test(name || "");
}

function hasGovernmentFlavor(result, structured) {
  const captionDefendant = structured.defendant || "";
  const parties = Array.isArray(result.party) ? result.party : [];
  const joined = `${captionDefendant} ${parties.join(" ")}`.toLowerCase();
  return (
    /governor|attorney general|secretary|commission|director|state of|people of|department|agency|board/.test(joined) ||
    (!looksLikeOrganization(captionDefendant) && parties.some((name) => !looksLikeOrganization(name) && name !== structured.plaintiff))
  );
}

function explainDisputeInPlainEnglish(result, structured, topic, motionShort) {
  const plaintiff = structured.plaintiff || "The plaintiff";
  const defendant = structured.defendant || "the defendant";
  const cause = `${result.cause || ""}`.toLowerCase();
  const nature = `${result.suitNature || ""}`.toLowerCase();
  const governmentFlavor = hasGovernmentFlavor(result, structured);
  const kalshiAsPlaintiff = /kalshi/.test(plaintiff.toLowerCase());

  if (kalshiAsPlaintiff && governmentFlavor && /commodit|exchange|market|trading/.test(`${cause} ${nature}`)) {
    return `${plaintiff} appears to be arguing that government officials should not be able to block, punish, or shut down one of its market products without running into federal law first.`;
  }

  if (/fraud/.test(`${cause} ${nature}`)) {
    return `The dispute appears to be about alleged fraud or misleading conduct tied to Kalshi.`;
  }

  if (/consumer|class action/.test(`${cause} ${nature}`)) {
    return `The dispute appears to be about whether Kalshi harmed a broader group of customers or users, not just one person.`;
  }

  if (/contract/.test(`${cause} ${nature}`)) {
    return `The dispute appears to be about whether Kalshi broke a deal or legal promise.`;
  }

  if (/statutory/.test(`${cause} ${nature}`)) {
    return `The dispute appears to be about whether Kalshi or the other side violated a specific law.`;
  }

  if (/preliminary injunction/i.test(motionShort) && /temporary restraining order/i.test(motionShort)) {
    return `${plaintiff} is asking the judge to step in quickly before the full case is finished.`;
  }

  return `The dispute appears to be about ${plainTopicLabel(topic)}.`;
}
...
function buildCaseOverview(result, structured) {
  ...
  lines.push(explainDisputeInPlainEnglish(result, structured, topic, motionShort));
...
function buildStructuredCourtFields(result, title, snippet) {
  ...
  const humanDisputeSummary = explainDisputeInPlainEnglish(
    result,
    {
    ...
    inferMatterTopic(result, leadDocumentDescription),
    cleanDocumentDescription(motionOrOrderDoc?.short_description || "")
  );
  ...
  const structured = {
    ...
    partyNames: Array.isArray(result.party) ? result.party.filter(Boolean) : [],
    plaintiff: sides?.plaintiff || "",
    defendant: sides?.defendant || "",
    sourcePublishedAt: sourcePublishedAt ? new Date(sourcePublishedAt).toISOString() : "",
    jurisdictionType: normalizeText(result.jurisdictionType || ""),
    cause: normalizeText(result.cause || ""),
    suitNature: normalizeText(result.suitNature || ""),
    legalPosture,
