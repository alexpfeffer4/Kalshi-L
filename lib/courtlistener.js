...
function summarizeCourtListenerMatter({ title, snippet, type, severity, structured }) {
  ...
  if (type === "legal_loss") {
    return `CourtListener surfaced ${partyLabel}${postureLabel} in ${courtLabel}, suggesting Kalshi took a procedural hit. ${snippet}`;
  }

  if (severity === "major") {
    return `CourtListener surfaced ${partyLabel}${postureLabel} in ${courtLabel} with enough weight to merit immediate review. ${snippet}`;
  }

  return `CourtListener surfaced ${partyLabel}${postureLabel} in ${courtLabel}. ${snippet}`;
}
