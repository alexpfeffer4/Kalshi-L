...
export function eventBreakdown(event) {
  ...
  const parties = [event.plaintiff || details.plaintiff, event.defendant || details.defendant].filter(Boolean).join(" v. ");
  const court = event.court || details.court || details.courtCitationString || "";
  const docket = event.docketNumber || details.docketNumber || "";
  ...
  const caseOverview = details.caseOverview || "";
  ...
  const courtTypeCopy = {
    lawsuit:
      caseOverview
        ? caseOverview
        : parties && court
        ? `${parties} is the court fight here, with ${docket || "the docket"} parked in ${court}. The posture is ${posture || "active litigation"}, so the receipt is a real case record rather than a vague complaint blob.`
        : `This is a court-filed dispute involving Kalshi, with the docket making the story specific instead of generic.`,
    legal_loss:
      caseOverview
        ? `${caseOverview} The procedural posture now reads as ${posture || "an adverse ruling or denied motion"}, which is why the case is being tracked as a legal hit.`
        : parties && court
        ? `${parties} is the docket that produced the setback, and the posture is ${posture || "an adverse ruling or denied motion"}. This is the kind of court paper that matters because it tells you who moved, where, and who got clipped.`
        : `This is a court setback on the docket, not just a rumor cycle. The key issue is the actual ruling posture.`,
  };

  const courtSeverityCopy = {
    major:
      `For Kalshi this is meaningful courtroom pressure. ${sourcePublished ? `The source record surfaced on ${formatEventDate(sourcePublished)}.` : ""} It is the kind of filing that can change legal leverage, not just vibes.`,
    notable:
      `For Kalshi this is a real litigation swing that can move the narrative. ${sourcePublished ? `The source record surfaced on ${formatEventDate(sourcePublished)}.` : ""} It is not final doom, but it is not empty noise either.`,
    minor:
      `For Kalshi this is a smaller court bruise, but the docket still matters because it is tied to a named case and posture. ${sourcePublished ? `The source record surfaced on ${formatEventDate(sourcePublished)}.` : ""}`,
  };

  if (event.sourceType === "court") {
    return {
      whatHappened: courtTypeCopy[event.type] || courtTypeCopy.lawsuit,
      whatItMeans: courtSeverityCopy[event.severity] || courtSeverityCopy.notable,
    };
  }
