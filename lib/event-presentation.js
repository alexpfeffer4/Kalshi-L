...
export function sourceDetailsRows(event) {
  ...
  if (event.sourceType === "court") {
    if (details.plaintiff || details.defendant) {
      rows.push({ label: "Parties", value: [details.plaintiff, details.defendant].filter(Boolean).join(" v. ") });
    }
    if (details.docketNumber) rows.push({ label: "Docket", value: details.docketNumber });
    if (details.court) rows.push({ label: "Court", value: details.court });
    if (details.courtCitationString) rows.push({ label: "Citation", value: details.courtCitationString });
    if (details.filedDate) rows.push({ label: "Filed", value: formatEventDate(details.filedDate) });
    if (details.snippet) rows.push({ label: "Docket snippet", value: details.snippet });
  }

  if (event.sourceType === "regulator") {
    if (details.agency) rows.push({ label: "Agency", value: details.agency });
    if (details.source) rows.push({ label: "Source lane", value: details.source });
    if (details.publishedAt) rows.push({ label: "Published", value: formatEventDate(details.publishedAt) });
