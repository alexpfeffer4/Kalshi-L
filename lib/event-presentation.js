...
function summarizeLeadDocForHumans(event) {
  const details = event.sourceDetails || {};
  const plaintiff = event.plaintiff || details.plaintiff || "The plaintiff";
  const defendant = event.defendant || details.defendant || "the other side";
  const raw = `${details.leadDocumentDescription || ""} ${details.laterDocumentDescription || ""}`;
  const lower = raw.toLowerCase();

  if (!lower.trim()) return "";

  if (lower.includes("preliminary injunction") && lower.includes("temporary restraining order")) {
    return `${plaintiff} sued ${defendant} and quickly asked the judge for emergency relief while the full case plays out.`;
  }

  if (lower.includes("preliminary injunction")) {
    return `${plaintiff} sued ${defendant} and asked the judge to temporarily block something while the case is pending.`;
  }

  if (lower.includes("temporary restraining order") || lower.includes("tro")) {
    return `${plaintiff} sued ${defendant} and asked the judge for an emergency short-term order right away.`;
  }

  if (lower.includes("motion to dismiss") || lower.includes("dismiss")) {
    return `${plaintiff} sued ${defendant}, and one side is already trying to get the case thrown out early.`;
  }

  if (lower.includes("transfer")) {
    return `${plaintiff} sued ${defendant}, and someone is trying to move the case to a different court.`;
  }

  if (lower.includes("summary judgment")) {
    return `${plaintiff} sued ${defendant}, and one side is asking the judge to decide the case without a full trial.`;
  }
...
function isBadStoredCourtOverview(value) {
  const lower = (value || "").toLowerCase();
  if (!lower) return true;
  return (
    lower.includes("the case is tagged in courtlistener") ||
    lower.includes("a later docket event says") ||
    lower.includes("a later case number event says") ||
    lower.includes("jurisdiction:") ||
    lower.includes("legal basis for being in this court") ||
    lower.includes("cause:") ||
    lower.includes("type of case:") ||
    lower.includes("nature of suit:")
  );
}

function buildFallbackCourtOverview(event) {
  const details = event.sourceDetails || {};
  const plaintiff = event.plaintiff || details.plaintiff || "A party";
  const defendant = event.defendant || details.defendant || "the other side";
  const court = event.court || details.court || details.courtCitationString || "court";
  const leadActionSummary = details.leadActionSummary || "";
  const humanDisputeSummary = details.humanDisputeSummary || "";
  const caseTopic = details.caseTopic || "";
  const posture = event.legalPosture || details.legalPosture || "";
  const stage = plainLanguagePosture(posture);

  const lines = [];
  lines.push(`${plaintiff} sued ${defendant} in ${court}.`);
  if (leadActionSummary) lines.push(leadActionSummary);
  if (humanDisputeSummary) lines.push(humanDisputeSummary);
  if (caseTopic) lines.push(caseTopic);
  if (posture) lines.push(`Right now, ${stage}.`);
  return asPlainExplanation(lines.join(" "));
}
...
function buildPlainCourtWhatHappened(event) {
  ...
  const caseOverview = isBadStoredCourtOverview(details.caseOverview) ? "" : details.caseOverview || "";
  const leadActionSummary = details.leadActionSummary || "";
  const humanDisputeSummary = details.humanDisputeSummary || "";
  const humanLeadDocSummary = summarizeLeadDocForHumans(event);
  ...
  if (caseOverview) {
    return asPlainExplanation(caseOverview);
  }

  if (leadActionSummary || humanDisputeSummary || details.caseTopic) {
    return buildFallbackCourtOverview(event);
  }

  if (humanLeadDocSummary) {
    return asPlainExplanation(`${humanLeadDocSummary} The case is in ${court}.`);
  }

  if (leadActionSummary) {
    return asPlainExplanation(`${leadActionSummary} The case is in ${court}.`);
  }

  if (leadDoc && parties && !/motion by plaintiff|attachments:|declaration|memorandum of law/i.test(leadDoc)) {
    return asPlainExplanation(
...
export function courtSpecificLead(event) {
  ...
  if (caseOverview) {
    return asPlainExplanation(`${caseOverview} Right now, ${stage}.${publishedLine}`);
  }

  if (leadActionSummary || humanDisputeSummary || details.caseTopic) {
    return asPlainExplanation(`${buildFallbackCourtOverview(event)} The source record surfaced on ${formatEventDate(sourcePublished)}.`.trim());
  }

  if (humanLeadDocSummary) {
    return asPlainExplanation(`${humanLeadDocSummary} Right now, ${stage}.${publishedLine}`);
  }

  if (leadActionSummary) {
    return asPlainExplanation(`${leadActionSummary} Right now, ${stage}.${publishedLine}`);
  }

  if (sides && kalshiIsDefendant && !/motion by plaintiff|attachments:|declaration|memorandum of law/i.test(leadDoc)) {
    return asPlainExplanation(
  ...
  if (sides && kalshiIsPlaintiff && !/motion by plaintiff|attachments:|declaration|memorandum of law/i.test(leadDoc)) {
    return asPlainExplanation(
