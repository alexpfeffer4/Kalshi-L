...
function regulatorSummary(title, description, source) {
  ...
  if (/investigation|probe/.test(lower)) {
    return asSlangBlurb(`${title}. This reads like regulator side-eye with actual paperwork energy.`);
  }

  if (/enforcement|charges|order|settlement|sues/.test(lower)) {
    return asSlangBlurb(`${title}. This is not random chatter. Kalshi may be catching real agency smoke here.`);
  }

  return asSlangBlurb(`${title}. Government-lane attention is usually where the vibes stop being funny.`);
}
...
function regulatorWhyItMatters(lower) {
  if (/investigation|probe|enforcement|charges|sues|order|settlement/.test(lower)) {
    return prefixWhyItMatters(
      "this is official regulator heat, which means the problem is no longer just PR. It can become real compliance pain, legal pressure, or business restrictions."
    );
  }

  return prefixWhyItMatters(
    "once government or regulator sources enter the chat, the story gets way more serious than ordinary commentary or repost farming."
  );
}

function inferRegulatoryPosture(lower) {
  if (/investigation|probe/.test(lower)) return "investigation";
  if (/enforcement|charges|order|settlement/.test(lower)) return "enforcement action";
  if (/review|oversight|agency/.test(lower)) return "official review";
  return "official source item";
}
...
export function classifyRegulatorResult(result) {
  ...
  return {
    ...
    candidate: {
      ...
      agency: /cftc/i.test(lower)
        ? "CFTC"
        : /sec/i.test(lower)
          ? "SEC"
          : /ftc/i.test(lower)
            ? "FTC"
            : /justice/i.test(lower)
              ? "DOJ"
              : source || "",
      legalPosture: inferRegulatoryPosture(lower),
      sourceDetails: {
        agency: /cftc/i.test(lower)
          ? "CFTC"
          : /sec/i.test(lower)
